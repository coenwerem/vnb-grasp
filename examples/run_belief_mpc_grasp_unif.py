...
Usage:
    # Basic run with cube ; default
    python examples/run_belief_mpc_grasp.py --steps 100

    # With YCB objects from the paper
    python examples/run_belief_mpc_grasp.py --object mustard --steps 100
    python examples/run_belief_mpc_grasp.py --object soup --steps 100
    python examples/run_belief_mpc_grasp.py --object tennis_ball --steps 100
    python examples/run_belief_mpc_grasp.py --object potted_meat --steps 100

    # With video recording
    python examples/run_belief_mpc_grasp.py --steps 100 --record --object mustard

    # Custom risk parameters
    python examples/run_belief_mpc_grasp.py --steps 100 --beta 0.95 --lambda-cvar 0.7 --particles 200

    # List available objects
    python examples/run_belief_mpc_grasp.py --list-objects

    # NOTE: IK fails with this variant. DON'T USE.
...
class VideoRecorder:
    ...
    def _draw_sparkline(
        ...
        y_coords = (
            y + height - ((values - min_val) / (max_val - min_val + 1e-6)) * height
        ).astype(int)
    ...
    def capture_frame_with_force(
        self,
        force_direction: float = 0,
    ...
    def save(self):
        ...
        print(
            f"Saving video to {self.output_path} ({len(self.frames)} frames @ {self.width}x{self.height}, {self.fps}fps)"
        )
...
def setup_env(object_name: str = "cube") -> RawMujocoEnv:
    """Initialize MuJoCo environment with ZArm + RealHand L6 and specified object.

    Args:
        object_name: Object to grasp. Options: cube, mustard, soup, tennis_ball, potted_meat

    Returns:
        Configured RawMujocoEnv
    """
    xml_path = "arenas/zarm_realhand_l6_right_arena/scene.xml"

    # Get object configuration
    try:
        obj_config = get_object_config(object_name)
    except ValueError as e:
        print(f"Warning: {e}. Falling back to cube.")
        obj_config = get_object_config("cube")

    # Determine which collision geom to use
    object_geoms = [obj_config.collision_geom]

    print(f"Loading scene: {xml_path}")
    print(f"Target object: {object_name} ({obj_config.body_name})")
    ...
    # Store object config on env for later use
    env.object_config = obj_config
    ...
    print(f"  Object geom: {object_geoms[0]}")
...
def compute_arm_ik(
    target_pos_world: np.ndarray, target_orient: np.ndarray, q0: np.ndarray = None
) -> Optional[np.ndarray]:
...
def get_grasp_target_pose(
    env: RawMujocoEnv, object_name: str = "cube", height_offset: float = 0.12
):
    ...
    target_rot = np.array([[-1, 0, 0], [0, 1, 0], [0, 0, -1]], dtype=np.float64)

    return target_pos, target_rot


def position_arm_for_grasp(
    env: RawMujocoEnv,
    object_name: str = "cube",
    settling_steps: int = 200,
    use_pregrasp: bool = True,
    use_graspit_offset: bool = True,
    pregrasp_fraction: float = None,
):
    """Position the arm and object for grasping using GraspIt pregrasp pose.

    NEW APPROACH (user-requested fix):
    1. Place object at a known location in the world
    2. Compute desired palm pose from GraspIt pregrasp (accounting for transformations)
    3. Use IK to solve for arm configuration
    4. Check for collisions
    5. If collision-free, execute grasp

    This ensures the hand is properly positioned with respect to the object
    before grasp execution begins.

    Args:
        env: MuJoCo environment
        object_name: Name of object to grasp ("cube", "mustard", "soup", etc.)
        settling_steps: Number of simulation steps to let arm settle
        use_pregrasp: If True, load pregrasp hand config from GraspIt database
        use_graspit_offset: If True, use GraspIt object pose relative to palm
        pregrasp_fraction: Fraction of GraspIt hand target used for initial hand pose
    ...
    # Use object-specific pregrasp fraction if not provided
    if pregrasp_fraction is None:
        pregrasp_fraction = obj_config.pregrasp_fraction

    print(f"  Object strategy: {obj_config.grasp_strategy.value}")
    print(f"  Pregrasp fraction: {pregrasp_fraction}")
    print(f"  Palm height offset: {obj_config.palm_height_offset}m")
    ...
    # Find object body
    obj_body_name = obj_config.body_name
    obj_body_id = mj.mj_name2id(env.model, mj.mjtObj.mjOBJ_BODY, obj_body_name)
    if obj_body_id < 0:
        obj_body_id = mj.mj_name2id(env.model, mj.mjtObj.mjOBJ_BODY, object_name)
        if obj_body_id < 0:
            print(f"  ERROR: Object body '{obj_body_name}' not found")
            return False

    obj_jnt_adr = env.model.body_jntadr[obj_body_id]
    obj_qpos_adr = env.model.jnt_qposadr[obj_jnt_adr]

    # STEP 1: Place object at a known location in the world
    # Use a standard table position with good reachability
    obj_world_pos = np.array([0.0, 1.2, obj_config.table_z + obj_config.half_height])
    obj_world_quat = obj_config.upright_quat

    print(
        f"  Placing object at world position: [{obj_world_pos[0]:.3f}, {obj_world_pos[1]:.3f}, {obj_world_pos[2]:.3f}]"
    )

    # Move ALL non-target objects far away to avoid interference
    all_ycb_bodies = [
        "006_mustard_bottle",
        "005_tomato_soup_can",
        "056_tennis_ball",
    ...
    for other_body_name in all_ycb_bodies:
        if other_body_name == obj_body_name:
            continue
        other_body_id = mj.mj_name2id(env.model, mj.mjtObj.mjOBJ_BODY, other_body_name)
        if other_body_id >= 0:
            other_jnt_adr = env.model.body_jntadr[other_body_id]
            if other_jnt_adr >= 0:
                other_qpos_adr = env.model.jnt_qposadr[other_jnt_adr]
                env.data.qpos[other_qpos_adr : other_qpos_adr + 3] = [10.0, 10.0, -1.0]

    # Place target object
    env.data.qpos[obj_qpos_adr : obj_qpos_adr + 3] = obj_world_pos
    env.data.qpos[obj_qpos_adr + 3 : obj_qpos_adr + 7] = obj_world_quat
    env.data.qvel[:] = 0.0
    mj.mj_forward(env.model, env.data)

    # STEP 2: Load GraspIt grasp configuration
    grasp_config = None
    pregrasp_hand_q = None
    grasp_object_offset = None
    grasp_object_quat = None

    if use_pregrasp and object_name != "cube":
        grasp_config = get_full_grasp_config(
            object_name, grasp_index=0, metric="epsilon"
        )
        if grasp_config is not None:
            print(f"  Loaded GraspIt grasp for {object_name}")
            print(f"    Epsilon quality: {grasp_config['epsilon_quality']:.4f}")
            print(f"    N contacts: {grasp_config['n_contacts']}")
            print(f"    Object offset (m): {grasp_config['object_pos']}")

            pregrasp_hand_q = grasp_config["hand_q"] * pregrasp_fraction

            if use_graspit_offset:
                grasp_object_offset = grasp_config["object_pos"]
                grasp_object_quat = grasp_config["object_quat"]

                offset_norm = float(np.linalg.norm(grasp_object_offset))
                if not np.isfinite(offset_norm) or offset_norm > 0.20:
                    print("  GraspIt offset looks invalid; falling back to default")
                    grasp_object_offset = None
                    grasp_object_quat = None
        else:
            print(f"  No grasp data available for {object_name}, using open hand")

    # STEP 3: Compute desired palm pose using object-specific strategy
    obj_world_pos = env.data.qpos[obj_qpos_adr : obj_qpos_adr + 3].copy()
    obj_world_quat = env.data.qpos[obj_qpos_adr + 3 : obj_qpos_adr + 7].copy()

    palm_world_pos, palm_world_rot = compute_strategy_specific_palm_pose(
        obj_world_pos,
        obj_world_quat,
        obj_config,
        grasp_offset=grasp_object_offset,
        grasp_quat=grasp_object_quat,
    )

    print(f"  Strategy-specific palm pose computed:")
    print(
        f"    Position: [{palm_world_pos[0]:.3f}, {palm_world_pos[1]:.3f}, {palm_world_pos[2]:.3f}]"
    )

    # STEP 4: Compute IK for desired palm pose
    print(f"  Computing IK for target pose...")

    # Try IK with current arm config as seed
    grasp_arm_config = compute_arm_ik(
        palm_world_pos, palm_world_rot, q0=env.data.qpos[0:6].copy()
    )

    if grasp_arm_config is None:
        print("  IK failed with current config, trying standard seeds...")
        # Try with a few standard configurations
        seed_configs = [
            np.array(
                [-0.826, -2.200, -1.643, -1.429, 0.500, 2.090]
            ),  # Original collision-free config
            np.array([0.0, -1.8, -1.5, -1.5, 0.0, 0.0]),  # Standard overhead config
            np.array([-0.5, -2.0, -1.5, -1.3, 0.3, 1.57]),  # Alternative
        ]
        for seed in seed_configs:
            grasp_arm_config = compute_arm_ik(palm_world_pos, palm_world_rot, q0=seed)
            if grasp_arm_config is not None:
                break

    if grasp_arm_config is None:
        print("  ERROR: IK failed to find solution for target pose")
        return False

    print(f"  IK solution: {[f'{q:.3f}' for q in grasp_arm_config]}")

    # STEP 5: Set arm configuration and check for collisions
    env.data.qpos[0:6] = grasp_arm_config
    if pregrasp_hand_q is not None:
        env.data.qpos[6:17] = pregrasp_hand_q
    else:
        env.data.qpos[6:17] = 0.0
    env.data.qvel[:] = 0.0
    mj.mj_forward(env.model, env.data)

    # Check for collisions ; excluding object-hand contacts which are expected
    hand_object_collision = False
    ...
    for c in range(env.data.ncon):
        ...
        # Check for premature hand-object collisions
        is_hand_geom = any(
            kw in n1 or kw in n2 for kw in ["hand", "palm", "finger", "thumb"]
        )
        is_obj_geom = obj_body_name in n1 or obj_body_name in n2
        if is_hand_geom and is_obj_geom:
            hand_object_collision = True

    if table_collision:
        print(f"  WARNING: Hand-table collision detected in IK solution")

    if hand_object_collision:
        print(f"  WARNING: Premature hand-object collision in pregrasp pose")
        # This might be acceptable depending on pregrasp_fraction

    # Report pose achieved
    actual_palm_pos = env.data.xpos[palm_body_id].copy()
    palm_error = np.linalg.norm(actual_palm_pos - palm_world_pos)
    print(
        f"  Achieved palm position: [{actual_palm_pos[0]:.3f}, {actual_palm_pos[1]:.3f}, {actual_palm_pos[2]:.3f}]"
    )
    print(f"  Position error: {palm_error * 1000:.1f}mm")

    # STEP 6: Settle the arm with position control
    print(f"  Settling arm for {settling_steps} steps...")
    for _ in range(settling_steps):
        env.data.ctrl[0:6] = grasp_arm_config
        if pregrasp_hand_q is not None:
            env.data.ctrl[6:17] = pregrasp_hand_q
        else:
            env.data.ctrl[6:17] = 0.0
        mj.mj_step(env.model, env.data)

    # Final state report
    final_palm_pos = env.data.xpos[palm_body_id].copy()
    final_obj_pos = env.data.qpos[obj_qpos_adr : obj_qpos_adr + 3].copy()

    print(
        f"  Final palm position: [{final_palm_pos[0]:.3f}, {final_palm_pos[1]:.3f}, {final_palm_pos[2]:.3f}]"
    )
    print(
        f"  Final object position: [{final_obj_pos[0]:.3f}, {final_obj_pos[1]:.3f}, {final_obj_pos[2]:.3f}]"
    )
    print(
        f"  Palm-object distance: {np.linalg.norm(final_palm_pos - final_obj_pos) * 1000:.1f}mm"
    )
    ...
    print(f"  Hand-object contacts: {hand_object_contacts}")
    print(f"  Table contacts: {table_contacts}")
    print("  Arm positioned for grasp execution")
...
def run_belief_mpc(
    ...
    if not position_arm_for_grasp(
        env,
        object_name,
        settling_steps=300,
        use_pregrasp=use_pregrasp,
        use_graspit_offset=use_graspit_offset,
        pregrasp_fraction=pregrasp_fraction,
    ):
    ...
    for step in range(max_steps):
        ...
        if (
            step >= min_steps_for_termination
            and metrics.n_contacts >= config.min_contacts
            and len(planner.entropy_history) >= 3
        ):
            recent_delta = abs(
                planner.entropy_history[-1] - planner.entropy_history[-3]
            )
    ...
    success = (
        (final_gws.is_force_closure and final_gws.epsilon > 0.1)
        or (final_contact_quality >= 0.7 and final_gws.volume > 5.0)
        or (final_gws.n_contacts >= 5 and final_gws.volume > 10.0)
    )
    ...
    summary = RunSummary(
        ...
        final_normalized_entropy=final_entropy / max_entropy
        if max_entropy > 0
        else 0.0,
...
def run_wrist_lift_test(
    ...
    lift_ratio = (
        actual_cube_lift / actual_palm_lift if actual_palm_lift > 0.001 else 0.0
    )
    ...
    print(f"    Success: {'[YES]: PASSED' if success else '[NO]: FAILED'}")
    print("=" * 70)
...
def run_3pulse_shear_test(
    ...
    shear_pulses: Tuple[float, float, float] = (
        3.0,
        6.0,
        12.0,
    ),  # Small/medium/large forces ; N
    ...
    print(f"  Target lift: {lift_height:.3f}m over {lift_duration:.1f}s")
    print(
        f"  Shear pulses: {shear_pulses[0]:.1f}N (small), {shear_pulses[1]:.1f}N (medium), {shear_pulses[2]:.1f}N (large)"
    )
    ...
    for pulse_idx, (pulse_force, pulse_name) in enumerate(
        zip(shear_pulses, pulse_names)
    ):
        ...
        print(
            f"    {pulse_name}: max_disp={max_disp_this_pulse * 1000:.1f}mm, slip={pulse_slip * 1000:.1f}mm, drop={pulse_drop * 1000:.1f}mm"
        )
...
def run_wrist_lift_test_with_perturbation(
    ...
    for step_i in range(n_lift_steps):
        ...
        if step_i % 10 == 0:
            cube_pos_history.append(
                env.data.qpos[cube_qpos_adr : cube_qpos_adr + 3].copy()
            )
            palm_z_history.append(env.data.xpos[palm_body_id][2])
    ...
    return {
        ...
        # Z histories for plotting
        "cube_z_history": [float(pos[2]) for pos in cube_pos_history],
        "palm_z_history": [float(z) for z in palm_z_history],
...
def main():
    parser = argparse.ArgumentParser(
        description="Belief-MPC grasping with ZArm + RealHand",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Basic run with cube ; default object
  python examples/run_belief_mpc_grasp.py --steps 100

  # Run with YCB objects from the paper
  python examples/run_belief_mpc_grasp.py --object mustard --steps 100
  python examples/run_belief_mpc_grasp.py --object soup --steps 100
  python examples/run_belief_mpc_grasp.py --object tennis_ball --steps 100
  python examples/run_belief_mpc_grasp.py --object potted_meat --steps 100
  ...
  python examples/run_belief_mpc_grasp.py --list-objects
    ...
    parser.add_argument(
        ...
        type=str,
        default="cube",
        help="Object to grasp: cube, mustard, soup, tennis_ball, potted_meat",
    )
    parser.add_argument(
        "--list-objects", action="store_true", help="List available objects and exit"
    )
    parser.add_argument(
        "--particles", type=int, default=100, help="Number of belief particles"
    )
    ...
    parser.add_argument(
        "--lambda-cvar", type=float, default=0.5, help="CVaR weight in score"
    )
    parser.add_argument(
        "--delta", type=float, default=0.05, help="Failure probability bound"
    )
    parser.add_argument("--record", action="store_true", help="Record video of the run")
    parser.add_argument(
        "--output-dir", default="outputs/belief_mpc_runs", help="Output directory"
    )
    ...
    parser.add_argument(
        "--no-early-stop", action="store_true", help="Disable early termination"
    )
    parser.add_argument(
        "--no-pregrasp",
        action="store_true",
        help="Don't use GraspIt pregrasp (start with open hand)",
    )
    parser.add_argument(
        "--pregrasp-fraction",
        type=float,
        default=0.6,
        help="Fraction of GraspIt hand target to start from (default: 0.6)",
    )
    parser.add_argument(
        "--no-graspit-offset",
        action="store_true",
        help="Ignore GraspIt object offset/orientation and place object under palm",
    )
    parser.add_argument(
        "--lift-test",
        action="store_true",
    ...
    if args.list_objects:
        print("\nAvailable objects:")
        for name in list_available_objects():
            config = get_object_config(name)
            print(f"  {name:15} -> {config.ycb_id} (body: {config.body_name})")
        return
    ...
    try:
        obj_config = get_object_config(object_name)
        print(f"\nTarget object: {object_name} ({obj_config.ycb_id})")
    except ValueError as e:
        print(f"Error: {e}")
        print("Use --list-objects to see available objects")
        return

    env = setup_env(object_name=object_name)
    ...
    if args.record:
        ...
        output_dir.mkdir(parents=True, exist_ok=True)
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        video_path = str(output_dir / f"belief_mpc_{object_name}_{timestamp}.mp4")
    ...
    try:
        summary, arm_q_target, hand_q_target = run_belief_mpc(
            ...
            object_name=object_name,
            recorder=recorder,
            pregrasp_fraction=args.pregrasp_fraction,
            use_graspit_offset=not args.no_graspit_offset,
            use_pregrasp=not args.no_pregrasp,
        )
    ...
    print("=" * 70)
    print(f"[x] Object: {object_name} ({obj_config.body_name})")
